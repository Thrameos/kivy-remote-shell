...
# *****************************************************************************
import jpype
from jpype import JException, JProxy, JClass
...
class ExceptionTestCase(common.JPypeTestCase):
    ...
    # This test fails, no clear cause
...
#        self.assertEqual(name, e.__name__)
    ...
    def testExceptionInstanceof(self):
        e = JClass('jpype.exc.ChildTestException')()
        self.assertIsInstance(e, JClass('jpype.exc.ParentTestException'))

    def testExceptionPYEXCInstanceof(self):
        e = JClass('jpype.exc.ChildTestException')
        self.assertTrue(issubclass(e, JClass('jpype.exc.ParentTestException')))

    def testThrowChildExceptionFromCatchJExceptionParentClass(self):
        try:
            JClass('jpype.exc.ExceptionTest').throwChildTestException()
            self.fail()
        except JClass('jpype.exc.ParentTestException') as ex:
            self.assertIsInstance(ex, JClass('jpype.exc.ChildTestException'))

## Removed SD frames are added to the stack
#    def testCause(self):
...
#                break
    ...
    def testIndexError(self):
        with self.assertRaises(IndexError):
            raise JClass('java.lang.IndexOutOfBoundsException')("From Java")
