...
class NumericTestCase(common.JPypeTestCase):
    def testMathAbs(self):
        self.assertEqual(JClass('java.lang.Math').abs(-10), 10)

    def testDoubleConversion(self):
        f = JClass('java.lang.Float').MAX_VALUE * 2
        self.assertTrue(JClass("jpype.numeric.NumericTest").doubleIsTwiceMaxFloat(f))

    def testDoubleIsProperlyConverted(self):
        self.assertTrue(JClass('java.lang.Double').POSITIVE_INFINITY != 0.0)
        self.assertTrue(JClass('java.lang.Double').MAX_VALUE != 0.0)
        self.assertTrue(JClass('java.lang.Double').NaN != 0.0)
        self.assertTrue(JClass('java.lang.Double').NEGATIVE_INFINITY != 0.0)

    def testCompareNullLong(self):
        null = JObject(None, JClass('java.lang.Integer'))
    ...
    def testCompareNullFloat(self):
        null = JObject(None, JClass('java.lang.Double'))
        self.assertEqual(null, None)
        self.assertNotEqual(null, object())
        with self.assertRaisesRegex(TypeError, "null"):
            null > 0
    ...
    def testHashNull(self):
        ...
        null = JObject(None, JClass('java.lang.Float'))
        self.assertEqual(hash(null), hash(None))
